...
export function GithubActionsPanel({
  ...
  const handleTrigger = async () => {
    ...
    try {
      const result = await runCollectNow(loadGithubSettings(), (msg) => setProgress(msg));
      if (result.ok) {
      ...
      {open && (
        <div className="github-sync-form">
          <p className="github-sync-hint">
            监测 <code>collect.yml</code> 采集部署工作流（每 2 小时定时 + 可手动触发）。
            手动触发会在 GitHub 服务器上运行采集并更新 <code>store.json</code>。
          </p>
